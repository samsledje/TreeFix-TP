...
from treefix_tp.rasmus import tablelib
...
def msqerr(vals1, vals2):
    ...
    return mean([(vals1[i] - vals2[i]) ** 2 for i in xrange(len(vals1))])
...
def covmatrix(mat):
    """Covariance Matrix"""
    size = len(mat)
    return [[covariance(mat[i], mat[j]) for j in range(size)] for i in range(size)]


def corrmatrix(mat):
    """Correlation Matrix"""
    size = len(mat)
    return [[corr(mat[i], mat[j]) for j in range(size)] for i in range(size)]
...
def iter_window_index(x, xdist, esp=None):
    ...
    while highi + 1 < vlen:
        ...
        assert abs((high - low) - xdist) < 0.001, (low, high)
...
def rhyper(m, n, M, N, report=0):
    ...
    assert (type(m) == type(n) == type(M) == type(N) == int) and m <= n and m <= M and n <= N
...
def enrichItems(in_items, out_items, M=None, N=None, useq=True, extra=False):
    ...
    if extra:
        ...
        tab.add_col(
            "item_ratio",
            data=[row["in_count"] / float(row["in_count"] + row["out_count"]) for row in tab],
        )
        tab.add_col("size_ratio", data=[M / float(N) for row in tab])
        tab.add_col("fold", data=[row["item_ratio"] / row["size_ratio"] for row in tab])
...
def logNormalPdf(x, params):
    ...
    mu, sigma = params
    return 1 / (x * sigma * sqrt(2 * pi)) * exp(-((log(x) - mu) ** 2) / (2.0 * sigma ** 2))
...
def poissonCdf(x, params):
    ...
    if x < 0:
        ...
        return (gamma(floor(x + 1)) - gammainc(floor(x + 1), lambd)) / factorial(floor(x))
...
def gammaPdf(x, params):
    ...
    if x <= 0 or alpha <= 0 or beta <= 0:
        ...
        return (exp(-x * beta) * (x ** (alpha - 1)) * (beta ** alpha)) / gamma(alpha)
...
def negbinomPdf(k, r, p):
    return exp(gammaln(r + k) - gammaln(k + 1) - gammaln(r) + r * log(p) + k * log(1 - p))
...
def gamma(x):
    ...
    return ret * sqrt(2 * pi) / x * (x + 5.5) ** (x + 0.5) * exp(-x - 5.5)
...
def chiSquareFit(xbins, ybins, func, nsamples, nparams, minsamples=5):
    ...
    for i in xrange(len(xbins) - 1):
        expected.append((func(xbins[i]) + func(xbins[i + 1])) / 2.0 * sizes[i] * nsamples)
...
def plotfuncFit(func, paramsInit, xdata, ydata, start, end, step, plot=None, **options):
    from rasmus import gnuplot
...
def plotdistribFit(func, paramsInit, data, start, end, step, plot=None, **options):
    xdata, ydata = util.distrib(data, low=start, width=step)
    return plotfuncFit(func, paramsInit, xdata, ydata, start, end, step / 10, plot, **options)


def chi_square_fit(
    cdf, params, data, ndivs=20, minsamples=5, plot=False, start=-util.INF, end=util.INF
):
    ...
    expected.extend(
        [len(data) * (cdf(x[i + 1], params) - cdf(x[i], params)) for i in range(1, len(x) - 1)]
    )
...
def fit_distrib(cdf, params_init, data, ndivs=20, minsamples=5, start=-util.INF, end=util.INF):
...
def solveCubic(a, b, c, real=True):
    ...
    u2 = u1 * complex(-0.5, -sqrt(3) / 2)
    u3 = u1 * complex(-0.5, sqrt(3) / 2)
    ...
    if real:
        return [x.real for x in [root1, root2, root3] if abs(x.imag) < 1e-10]
    else:
